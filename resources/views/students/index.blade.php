...
@push('styles')
    <style>
        ...
        .class-card.active {
            box-shadow: 0 0 0 2px #1a6b5a, 0 4px 16px rgba(26, 107, 90, 0.15);
        }
...
@section('content')
    <div x-data="studentsPage()" x-init="init()">
        ...
        <div class="mx-4 md:mx-10 lg:mx-20 xl:mx-60 mt-8">
            ...
            {{-- ── Header Banner ───────────────────────────────────────────────── --}}
            <div class="rounded-2xl mb-6 px-6 py-5 flex flex-col sm:flex-row sm:items-center h-28 gap-4 fade-in transition-all duration-300 p-6"
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 mb-6">
                <a href="{{ route('students.index') }}"
                    class="class-card bg-white rounded-xl px-4 py-3 border border-gray-100 fade-in
              {{ !isset($selectedClass) ? 'active' : '' }}">
                    <div class="class-icon mb-3" style="background-color: #0000001a;">
                        <i data-lucide="graduation-cap" class="w-5 h-5" style="color: #000000;"></i>
                    </div>
                    <p class="text-2xs text-gray-500 mb-1">Semua Kelas</p>
                    <p class="text-xl font-bold text-gray-800">{{ $classes->sum('total_aktif') }}</p>
                </a>
                @foreach ($classes as $class)
                    <a href="{{ route('students.index', ['class' => $class->slug]) }}"
                        class="class-card bg-white rounded-xl px-4 py-3 border border-gray-100 fade-in
              {{ isset($selectedClass) && $selectedClass->id === $class->id ? 'active' : '' }}">
                        <div class="class-icon mb-3" style="background-color: {{ $class->color ?? '#000000' }}1a;">
                            <i data-lucide="graduation-cap" class="w-5 h-5"
                                style="color: {{ $class->color ?? '#000000' }};"></i>
                        </div>
                        <p class="text-2xs text-gray-500 mb-1">{{ $class->nama_kelas ?? 'N/A' }}</p>
                        <p class="text-xl font-bold text-gray-800">{{ $class->total_aktif ?? '0' }}</p>
                    </a>
                @endforeach
            </div>
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="border-b border-gray-100 bg-gray-50/50">
                                <th class="px-4 py-8  text-left w-10">
                                    <input type="checkbox" class="checkbox-custom" x-show="massalMode" x-cloak
                                        @change="toggleAll($event.target.checked)">
                                </th>
                                <th
                                    class="px-4 py-3 text-left text-sm font-semibold text-gray-500 uppercase tracking-wider w-12">
                                    No</th>
                                <th
                                    class="px-4 py-3 text-left text-sm font-semibold text-gray-500 uppercase tracking-wider">
                                    Nama Siswa</th>
                                <th
                                    class="px-4 py-3 text-left text-sm font-semibold text-gray-500 uppercase tracking-wider">
                                    Kelas</th>
                                ...
                                <th
                                    ...
                                    Status</th>
                                <th
                                    class="px-4 py-3 text-center text-sm font-semibold text-gray-500 uppercase tracking-wider">
                                    Aksi</th>
                            </tr>
                        </thead>
     ...
     MODAL: Tambah Cepat
════════════════════════════════════════════════════════ --}}
            <div x-show="showQuickAdd" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4"
                x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0"
                x-transition:enter-end="opacity-100">

                <div class="absolute inset-0 bg-black/40 backdrop-blur-sm" @click="showQuickAdd = false"></div>
